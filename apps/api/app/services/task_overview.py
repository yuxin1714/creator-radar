...
def task_overview(db):
    ...
    for creator in db.scalars(select(Creator).where(Creator.owner_id=='local-user').order_by(Creator.created_at.desc())):
    ...
    for model, kind, tab in [(Task, "metadata", None), (Transcript, "transcript", "transcript"), (Analysis, "analysis", "analysis")]:
        rows = db.execute(select(model, Work).join(Work, model.work_id == Work.id).where(model.owner_id == "local-user", Work.owner_id == "local-user").order_by(model.created_at.desc())).all()
    ...
    for run,work in db.execute(select(ResearchRun,Work).join(Work,Work.id==ResearchRun.work_id).where(ResearchRun.owner_id=='local-user',Work.owner_id=='local-user').order_by(ResearchRun.created_at.desc())):
