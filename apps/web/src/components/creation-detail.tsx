...
type Brief={platform:string;content_type:string;direction:string;style:string;playbook_id:string};type Generation={mode?:"draft"|"directions";id:string;status:string;content:string|null;error_summary:string|null;playbook_id:string;playbook_revision:string|null};type Project={work_id:string|null;output_language:string;id:string;title:string;idea:string|null;status:string;body:string|null;updated_at:string;brief:Brief|null;latest_generation:Generation|null};type Playbook={id:string;name:string;revision:string|null};
...
export function CreationDetail({projectId}:{projectId:string}){const [item,setItem]=useState<Project|null>(null),[skills,setSkills]=useState<Playbook[]>([]),[title,setTitle]=useState(""),[idea,setIdea]=useState(""),[body,setBody]=useState(""),[brief,setBrief]=useState<Brief>({platform:"tiktok",content_type:"knowledge",direction:"structure_borrowing",style:"professional",playbook_id:"structure-borrowing-v1"}),[message,setMessage]=useState(""),[busy,setBusy]=useState(false),[generating,setGenerating]=useState(false);
 ...
 async function generate(options:{mode:"draft"|"directions";direction_generation_id?:string;direction_index?:number}={mode:"draft"}){
   ...
   try{
     ...
     setPollEpoch(x=>x+1);
     setMessage(options.mode==="directions"?"正在按所选 Skill 生成三个方向。":"正在按所选 Skill 生成草稿。");
