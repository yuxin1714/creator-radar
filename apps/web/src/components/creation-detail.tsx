...
import { readRecovery, recoveryKey, type RecoveredDraft } from "@/lib/draft-recovery";
import { DraftConflict } from "@/components/draft-conflict";
import { useUnsavedDraft } from "@/hooks/use-unsaved-draft";
import { useSessionText } from "@/hooks/use-session-text";
...
export function CreationDetail({projectId}:{projectId:string}){const [item,setItem]=useState<Project|null>(null),[skills,setSkills]=useState<Playbook[]>([]),[title,setTitle]=useState(""),[idea,setIdea]=useState(""),[body,setBody]=useState(""),[brief,setBrief]=useState<Brief>({platform:"tiktok",content_type:"knowledge",direction:"structure_borrowing",style:"professional",playbook_id:"structure-borrowing-v1"}),[message,setMessage]=useState(""),[busy,setBusy]=useState(false),[generating,setGenerating]=useState(false);
 ...
 const {value:feedback,update:setFeedback,notice:feedbackNotice}=useSessionText(`creator-radar:feedback:v1:${projectId}`,5000);
 ...
 useEffect(()=>{
   ...
   try{
     if(dirty)sessionStorage.setItem(recoveryKey(projectId),JSON.stringify({title,idea,body,output_language:language,brief,base_updated_at:conflict?'unresolved':item.updated_at}));
     else sessionStorage.removeItem(recoveryKey(projectId));
     setRecoveryError("");
   }catch{setRecoveryError("浏览器暂存不可用，请及时保存草稿。");}
 },[projectId,item,dirty,recovery,title,idea,body,language,brief,conflict]);
 function recoverDraft(){
   if(!recovery)return;
   loadVersion(recovery);setRecovery(null);
   if(item&&recovery.base_updated_at!==item.updated_at)setConflict(item);
 ...
 useEffect(()=>{
   ...
   setItem(null);setRecovery(null);setConflict(null);
   async function load(){
     try {
       ...
       try{
         ...
         if(cached){const {base_updated_at,...snapshot}=cached;if(JSON.stringify(snapshot)!==JSON.stringify(baseline))setRecovery(cached);}
       }catch{setRecoveryError("浏览器暂存不可用，请及时保存草稿。");}
 ...
 async function save():Promise<boolean>{
   if(conflict){setMessage('请先完成版本对照。');return false;}
   if(archived){setMessage("请先恢复项目再编辑。");return false;}
   ...
   try{
     ...
     if(r.status===409&&d.code==='project_conflict'){
       const response=await fetch(`/api/creation-projects/${encodeURIComponent(projectId)}`,{cache:'no-store'});
       const latest=await response.json();
       if(!response.ok)throw new Error('最新版本读取失败，当前编辑仍保留。请重新点击保存以重试。');
       setConflict(latest);setMessage('请对照两个版本，确认后再保存。');return false;
     }
 ...
 if(!item&&!message)return <div className="detail-loading"><LoaderCircle className="spin"/>正在读取创作项目…</div>;if(!item)return <><Link className="back-link" href="/creation"><ArrowLeft size={15}/>返回创作空间</Link><section className="panel detail-error"><h1>无法打开项目</h1><p>{message}</p></section></>;const select=(label:string,key:keyof Brief,options:string[][])=><label>{label}<select value={brief[key]} onChange={e=>setBrief({...brief,[key]:e.target.value})}>{options.map(([value,text])=><option key={value} value={value}>{text}</option>)}</select></label>;const generation=item.latest_generation,running=generation?.status==="PENDING"||generation?.status==="PROCESSING";return <><Link className="back-link" href="/creation"><ArrowLeft size={15}/>返回创作空间</Link><section className="panel creation-editor">{recovery&&<div className="draft-recovery" role="status"><p>发现此项目的未保存草稿</p><Button variant="outline" onClick={recoverDraft}>恢复未保存草稿</Button><Button variant="ghost" onClick={discardRecovery}>丢弃暂存</Button></div>}{recoveryError&&<p role="alert" className="draft-recovery">{recoveryError}</p>}<div className="section-heading"><div><p className="eyebrow">CREATION DRAFT</p><h1>{item.title}</h1><p>草稿项目 · {item.status} · <span role="status">{busy?"正在保存":dirty?"未保存":"已保存"}</span></p></div><span className="neutral-label">{new Date(item.updated_at).toLocaleDateString("zh-CN")}</span></div>{item.work_id&&<div className="creation-reference"><span>参考作品已关联</span><Link href={`/works/${item.work_id}?tab=analysis`}>查看作品与分析</Link></div>}<ProjectStatus id={item.id} status={item.status} updatedAt={item.updated_at} disabled={!!conflict||busy||generating||!!running||dirty&&!archived} onChanged={data=>setItem(current=>current?{...current,...data}:current)}/>{conflict&&<DraftConflict key={conflict.updated_at} local={{title,idea,body,output_language:language,brief}} latest={conflict} archived={conflict.status==='ARCHIVED'} onResolve={snapshot=>{setItem(conflict);loadVersion(snapshot);setConflict(null);setMessage('对照结果已放回编辑器，请检查后保存。');}}/>}<fieldset className="project-fields" disabled={archived||!!conflict||!!conflict}><label>项目标题<input value={title} onChange={e=>setTitle(e.target.value)}/></label><label>创作想法<textarea value={idea} onChange={e=>setIdea(e.target.value)} placeholder="记录主题、受众或想表达的观点"/></label><div className="brief-grid"><label>输出语言<select aria-label="输出语言" value={language} onChange={e=>setLanguage(e.target.value)}><option value="zh-CN">中文</option><option value="en">English</option><option value="zh-en">中英双语</option></select></label>{select("目标平台","platform",platforms)}{select("内容类型","content_type",contentTypes)}{select("创作方向","direction",directions)}{select("表达风格","style",styles)}<label>创作 Skill<select value={brief.playbook_id} onChange={e=>setBrief({...brief,playbook_id:e.target.value})}>{brief.playbook_id!=="auto"&&brief.playbook_id!=="structure-borrowing-v1"&&!skills.some(skill=>skill.id===brief.playbook_id)&&<option value={brief.playbook_id} disabled>已停用或不可用：{brief.playbook_id}</option>}<option value="auto">自动匹配（按创作配置）</option><option value="structure-borrowing-v1">结构借鉴，观点重构</option>{skills.map(skill=><option key={skill.id} value={skill.id}>{skill.name}{skill.revision?` · ${skill.revision.slice(0,8)}`:""}</option>)}</select></label></div>{brief.playbook_id==="auto"&&<MatchingPreview platform={brief.platform} contentType={brief.content_type} direction={brief.direction} style={brief.style}/>}<label>正文草稿<textarea className="body-editor" value={body} onChange={e=>setBody(e.target.value)} placeholder="在这里编辑正文；生成结果需先采用，再保存到草稿"/></label></fieldset><div className="editor-actions"><ExportDraft title={title} body={body} workId={item.work_id}/><Button onClick={save} disabled={archived||!!conflict||busy||generating||!title.trim()}>{busy?<LoaderCircle className="spin" size={15}/>:<Save size={15}/>}保存草稿</Button><Button variant="outline" onClick={()=>void generate()} disabled={archived||!!conflict||busy||generating||running||!title.trim()}>{generating||running?<LoaderCircle className="spin" size={15}/>:<Sparkles size={15}/>}生成草稿</Button><Button variant="outline" onClick={()=>void generate({mode:"directions"})} disabled={archived||!!conflict||busy||generating||running||!title.trim()}>生成三个创作方向</Button>{message&&<span><Check size={15}/>{message}</span>}</div>{running&&<div className="generation-progress"><LoaderCircle className="spin" size={17}/>正在按已选择的 Skill 生成内容…</div>}{generation?.status==="FAILED"&&<p className="task-error">{generation.error_summary}</p>}{generation?.status==="COMPLETED"&&generation.content&&(generation.mode==="directions"?<CreationDirections content={generation.content} disabled={archived||!!conflict||busy||generating||!!running} onChoose={index=>void generate({mode:"draft",direction_generation_id:generation.id,direction_index:index})}/>:<section className="generation-preview"><div><strong>生成预览</strong><small>Skill {generation.playbook_id} · {generation.playbook_revision?.slice(0,8)||"本地版本"}</small></div><DraftPreview content={generation.content}/><Button disabled={archived||!!conflict} onClick={()=>setBody(generation.content||"")}>采用到正文</Button></section>)}<section className="generation-preview"><h2>反馈优化</h2><p>以编辑器当前正文为基础，按所选 Skill 和修改意见生成修订稿，原稿不会自动替换。</p>{feedbackNotice&&<p role="status">{feedbackNotice}</p>}<label>修改意见<textarea aria-label="修改意见" disabled={archived||!!conflict} value={feedback} maxLength={5000} onChange={e=>setFeedback(e.target.value)} placeholder="例如：开场更直接，保留结尾反转，减少产品介绍"/></label><Button variant="outline" disabled={archived||!!conflict||busy||generating||!!running||!body.trim()||!feedback.trim()||!title.trim()} onClick={()=>void generate({mode:"refine",feedback})}>按反馈优化正文</Button></section><GenerationHistory projectId={projectId} refresh={`${generation?.id}:${generation?.status}`} disabled={archived||!!conflict||busy||generating||!!running} onAdopt={setBody} onChoose={(id,index)=>void generate({mode:"draft",direction_generation_id:id,direction_index:index})}/><DraftHistory projectId={projectId} refresh={historyRefresh} onLoad={snapshot=>{if(!archived)loadVersion(snapshot)}} disabled={archived||!!conflict}/><div className="quiet-note">生成结果先保留为预览，只有点击“采用到正文”后才会替换编辑器中的内容。</div></section></>}
