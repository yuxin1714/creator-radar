...
export function WorkspacePage({ section }: { section: string }) {
  ...
  return <><Heading section={section} />
    ...
    : section === "creation" ? <><section className="panel"><div className="section-heading"><div><h2>继续创作</h2><p>这里属于你自己的作品，而不是收集的参考内容。</p></div><span className="neutral-label">创作功能待接入</span></div><Empty icon={<FileText size={28} />} title="还没有正在进行的创作" description="后续可从想法、参考作品或创作机会开始。项目将保存你的草稿与编辑进度。" /></section><div className="context-cards">{["从一个想法开始", "从参考作品开始", "从创作机会开始"].map((text, index) => <div className="panel context-card" key={text}><span className="eyebrow">0{index + 1}</span><h3>{text}</h3><p>入口预留 · 尚不可用</p></div>)}</div></>
