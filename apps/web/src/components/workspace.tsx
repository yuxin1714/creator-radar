...
function CreationPanel(){const [items,setItems]=useState<Array<{id:string;title:string;idea:string|null;status:string;updated_at:string}>>([]),[title,setTitle]=useState(""),[idea,setIdea]=useState(""),[busy,setBusy]=useState(false),[message,setMessage]=useState("");const load=()=>fetch("/api/creation-projects",{cache:"no-store"}).then(r=>r.json()).then(setItems).catch(()=>setMessage("暂时无法读取创作项目。"));useEffect(()=>{load()},[]);async function create(){if(!title.trim())return;setBusy(true);setMessage("");try{const r=await fetch("/api/creation-projects",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({title,idea:idea||null})});const d=await r.json();if(!r.ok)throw new Error(d.message);setTitle("");setIdea("");setMessage("草稿项目已创建。");load()}catch(e){setMessage(e instanceof Error?e.message:"无法创建项目。")}finally{setBusy(false)}}return <><section className="panel"><div className="section-heading"><div><h2>创作空间</h2><p>保存自己的想法和草稿，不与外部参考作品混淆。</p></div><span className="neutral-label">草稿模式</span></div><div className="creation-form"><input aria-label="项目标题" placeholder="项目标题" value={title} onChange={e=>setTitle(e.target.value)}/><textarea aria-label="创作想法" placeholder="先记录一个想法（可选）" value={idea} onChange={e=>setIdea(e.target.value)}/><Button disabled={busy||!title.trim()} onClick={create}>{busy?<LoaderCircle className="spin" size={15}/>:<Plus size={15}/>}新建草稿</Button>{message&&<p className="provider-message">{message}</p>}</div></section>{items.length?<section className="panel"><div className="section-heading"><div><h2>最近项目 <span className="neutral-label">{items.length}</span></h2></div></div><div className="data-list">{items.map(item=><div className="data-row creation-row" key={item.id}><div><Link href={`/creation/${item.id}`}><strong>{item.title}</strong></Link><span>{item.idea||"尚未记录内容"}</span></div><span>中文</span><span>{item.status}</span><span>{new Date(item.updated_at).toLocaleDateString("zh-CN")}</span></div>)}</div></section>:null}</>}
function SettingsPanel(){const [skills,setSkills]=useState<Array<{id:string;name:string;repository_url:string|null;revision:string|null;synced_at:string}>>([]),[message,setMessage]=useState(""),[syncing,setSyncing]=useState("");const load=()=>fetch("/api/playbooks",{cache:"no-store"}).then(r=>r.json()).then(setSkills).catch(()=>setMessage("暂时无法读取创作 Skill。"));useEffect(()=>{load()},[]);async function sync(id:string){setSyncing(id);setMessage("");try{const r=await fetch(`/api/playbooks/${id}/sync`,{method:"POST"}),d=await r.json();if(!r.ok)throw new Error(d.message);setMessage(d.updated?"已同步到最新版本。":"已经是最新版本。");load()}catch(e){setMessage(e instanceof Error?e.message:"同步失败。 ")}finally{setSyncing("")}}return <><section className="panel settings-panel"><h2>当前工作台</h2><dl><div><dt>运行方式</dt><dd>本机开发预览 · 不用于公网</dd></div><div><dt>账户与持久化</dt><dd>PostgreSQL 本机数据 · 单一 local-user</dd></div><div><dt>第三方凭证</dt><dd>仅在后端 .env 配置，不在页面填写或展示</dd></div></dl></section><section className="panel skill-settings"><div className="section-heading"><div><h2>创作 Skill 管理</h2><p>远程 Skill 同步后，新项目可选择最新版本；历史项目保留原版本。</p>{message&&<p className="provider-message">{message}</p>}</div></div>{skills.map(skill=><div className="skill-row" key={skill.id}><div><strong>{skill.name}</strong><span>{skill.repository_url}</span><small>版本 {skill.revision?.slice(0,8)} · 上次同步 {new Date(skill.synced_at).toLocaleString("zh-CN")}</small></div><Button variant="outline" disabled={syncing===skill.id} onClick={()=>sync(skill.id)}>{syncing===skill.id?<LoaderCircle className="spin" size={14}/>:null}同步</Button></div>)}</section></>}
