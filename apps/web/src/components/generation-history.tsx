...
export function GenerationHistory({projectId,refresh,disabled,onAdopt,onChoose}:{projectId:string;refresh:string;disabled:boolean;onAdopt:(content:string)=>void;onChoose:(id:string,index:number)=>void}){
 ...
 useEffect(()=>{
  ...
  async function load(){try{
   ...
   const r=await fetch(base+'?page='+page,{cache:'no-store',signal:controller.signal});const payload=await r.json();const data=Array.isArray(payload)?payload:payload.items;if(!r.ok||!Array.isArray(data))throw new Error(payload.message||'读取失败');
   const target=page===1?new URLSearchParams(window.location.search).get('generation'):null;
   ...
   if(!controller.signal.aborted){setPageInfo(Array.isArray(payload)?{...emptyPage,total:data.length}:payload);setItems(data);setSelected(current=>data.some((g:Generation)=>g.id===current)?current:target||data[0]?.id||'');}
 ...
 return <details className="draft-history" open={open} onToggle={e=>setOpen(e.currentTarget.open)}><summary>生成历史</summary>{loading?<p>正在读取生成记录…</p>:error?<p role="alert">{error}</p>:!items.length?<p>暂无生成记录。</p>:<><label>本页生成记录<select aria-label="生成记录" value={selected} onChange={e=>setSelected(e.target.value)}>{items.map(i=><option key={i.id} value={i.id}>{new Date(i.created_at).toLocaleString('zh-CN')} · {modes[i.mode]||i.mode} · {i.status==='COMPLETED'?'完成':i.status==='FAILED'?'失败':'处理中'}</option>)}</select></label>{item&&<div className="history-preview"><p>Skill {item.playbook_id} · {item.playbook_revision||'版本未记录'}</p>{item.error_summary&&<p role="alert">{item.error_summary}</p>}{item.status==='COMPLETED'&&item.content&&(item.mode==='directions'?<CreationDirections content={item.content} disabled={disabled} onChoose={index=>onChoose(item.id,index)}/>:<><DraftPreview content={item.content}/><Button variant="outline" disabled={disabled} onClick={()=>onAdopt(item.content!)}>载入此生成稿</Button><p>载入仅替换编辑器，保存后写入草稿版本。</p></>)}</div>}</>}{open&&<Pagination info={pageInfo} busy={loading} onPage={next=>{setSelected("");setPage(next)}}/>}</details>;
}
