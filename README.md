...
工作台入口：http://127.0.0.1:3000/today 。启动器会检查数据库、API、后台 worker 和前端，复用已运行的本项目服务。详细操作与故障排查见 [本地使用说明](docs/LOCAL_USE.md)。
...
- 保留原文的中英文译稿和分析、按语言查看历史；主要列表及历史分页。
- 项目完成/归档/恢复、修改意见与标签表单刷新恢复、草稿冲突对照和手动合并。
- 自定义 Skill、GitHub Skill 注册/同步、每日自动更新与匹配条件。
- 统一任务中心、本地持久化 Celery 队列；数据库备份和隔离恢复工具。
